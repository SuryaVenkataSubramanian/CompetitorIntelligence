...
async function handleRequest(req, res) {
  ...
  if (url === "/api/me") {
     ...
     */
    /* Which credential model is actually in force for THIS caller.
     *
     * A stored hash wins wherever one exists, so the mode is a property of the
     * account rather than of the deployment. It matters here because a stored
     * password cannot be displayed at all: scrypt is one-way by design, and
     * that is the point of storing a hash. Saying so is more useful than
     * returning null with no explanation. */
    const stored = auth.hasStoredHash(session.email);
    const mode = stored ? "stored" : (derivedAuth.available() ? "derived" : "none");
    ...
    return json(res, 200, {
      ...
      own_password_note:
        mode === "derived"
          ...
          : mode === "stored"
            ? "Generated by `npm run auth:init` and stored only as a scrypt hash, so it cannot be displayed — scrypt is one-way, which is the point of hashing it. A lost password means the deployment owner re-provisions, which changes all four."
            : "This deployment has no credentials configured.",
    });
  }
